...
class Speak extends EventEmitter {
	...
	speak(text, options) {
		if (!options) options = {};
		var profile = (options && options.profile)? options.profile : this.defaultProfile['default'];
	...
	say(text, options) {
		...
		if (options.intonate) {
			if (options.intonate === 'd' || options.intonate === 'default') profile = this.getVoice('default');
			if (options.intonate === 'l' || options.intonate === 'low') {
				profile = this.getVoice('low');
			}
			if (options.intonate === 'h' || options.intonate === 'high') profile = this.getVoice('high');
			if (options.intonate === 'f' || options.intonate === 'fast') profile = this.getVoice('fast');
			if (options.intonate === 's' || options.intonate === 'slow') profile = this.getVoice('slow');
		}
